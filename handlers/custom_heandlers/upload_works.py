...
@bot.callback_query_handler(func=lambda call: True, state=AdminState.upload_state_one)
def from_what_date_to_what_date(callback: CallbackQuery):
    ...
    bot.set_state(callback.from_user.id, AdminState.upload_state_two, callback.message.chat.id)
    bot.send_message(callback.from_user.id, f'Напиши год-месяц за который надо выгрузить файлы.\nПример: 2023-12')


@bot.message_handler(state=AdminState.upload_state_two)
